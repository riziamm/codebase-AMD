...
sys.path.append('/home/users/u5499379/Projects/ardes')
...
def evaluate_model_v2(args):
    """Evaluate a saved model or compare multiple models on specified evaluation data"""
    # Import evaluation functions here or ensure they are globally imported # Adjust import as needed

    # --- 1. Validate Input and Paths ---
    if not args.eval_data_path:
        raise ValueError("--eval_data_path is required for evaluate mode (specify path to test_data.pkl or test_test.pkl)")
    eval_data_file_path = Path(args.eval_data_path)
    if not eval_data_file_path.is_file():
         raise FileNotFoundError(f"Evaluation data file not found: {eval_data_file_path}")

    # --- 2. Load Evaluation Data and Prepare Summary ---
    print(f"\n--- Loading Evaluation Data for Summary ---")
    print(f"File: {eval_data_file_path}")
    eval_data = None
    data_summary = {} # Initialize dictionary to store summary
    try:
        ...
        if 'X_test' not in eval_data or 'y_test' not in eval_data:
             raise KeyError("Loaded evaluation data PKL file must contain 'X_test' and 'y_test' keys.")
        ...
        le = eval_data.get('le', None) # Safely get label encoder
        if le:
             try:
                 class_names = le.inverse_transform(sorted_classes)
                 for class_idx, class_name in zip(sorted_classes, class_names):
                      ...
                      percentage = (count / total_samples) * 100
                      print(f"  Class '{class_name}' (Encoded: {class_idx}): {count} samples ({percentage:.1f}%)")
                      data_summary['class_distribution'][int(class_idx)] = {'name': class_name, 'count': count, 'percentage': round(percentage, 2)}
             except Exception as le_err:
                 print(f"  Warning: Could not use label encoder ({le_err}). Showing encoded labels only.")
                 le = None # Fallback
        if not le: # Print/Store with encoded labels only
             for class_idx in sorted_classes:
                 ...
                 percentage = (count / total_samples) * 100
                 print(f"  Class {class_idx}: {count} samples ({percentage:.1f}%)")
                 data_summary['class_distribution'][int(class_idx)] = {'name': f'Class_{class_idx}', 'count': count, 'percentage': round(percentage, 2)}
        print("-----------------------------")
        ...
        traceback.print_exc()
        sys.exit(1)
    # --- END: Load Evaluation Data and Prepare Summary ---

    # Initialize results variable
    results = {}

    # --- 3. Perform Evaluation (Single or Multi-Model) ---
    if args.model_paths:
        # --- Compare multiple models ---
        print(f"\nComparing models: {args.model_paths}")

        # Balancing logic (assuming check_and_balance accepts data arrays)
        X_eval_comp = eval_data['X_test']
        y_eval_comp = eval_data['y_test']
        if args.balance_test:
            print("Balancing evaluation data for comparison...")
            try:
                X_eval_comp, y_eval_comp = check_and_balance_test_data(
                    X_test=X_eval_comp, y_test=y_eval_comp,
                    min_samples_per_class=args.min_samples_per_class,
                    balance_method=args.balance_method
                )
                print("Data balanced for comparison.")
                # Update eval_data dictionary if needed by compare_models
                eval_data['X_test'] = X_eval_comp
                eval_data['y_test'] = y_eval_comp
            except Exception as balance_err:
                 print(f"WARNING: Error during balancing for comparison: {balance_err}. Proceeding with unbalanced data.")

        # Call compare_models (assuming it accepts eval_data dictionary)
        try:
            results = compare_models(
                 model_paths=args.model_paths,
                 eval_data=eval_data, # Pass the potentially updated data dict
                 report_dir=args.report_dir
            )
        except TypeError as type_err:
             print(f"\nERROR: 'compare_models' function might need update to accept 'eval_data' dictionary. Details: {type_err}")
             results = {}
        except Exception as comp_err:
             print(f"\nERROR during model comparison: {comp_err}")
             traceback.print_exc()
             results = {}
        ...
        # Assign loaded data
        X_eval_single = eval_data['X_test']
        y_eval_single = eval_data['y_test']
        feature_names_single = eval_data.get('feature_names')
        le_single = eval_data.get('le')

        # Handle balancing (assuming check_and_balance accepts data arrays)
        if args.balance_test:
            print("Balancing evaluation data...")
            try:
                X_eval_single, y_eval_single = check_and_balance_test_data(
                    X_test=X_eval_single, y_test=y_eval_single,
                    min_samples_per_class=args.min_samples_per_class,
                    balance_method=args.balance_method
                )
                print("Data balanced for evaluation.")
            except Exception as balance_err:
                 print(f"WARNING: Error during balancing: {balance_err}. Proceeding with unbalanced data.")

        # Call evaluate_saved_model (assuming it accepts data arrays/components)
        try:
            results = evaluate_saved_model(
                model_path=args.model_path,
                X_test=X_eval_single,
                y_test=y_eval_single,
             ...
             print(f"\nERROR: 'evaluate_saved_model' function might need update to accept X_test, y_test, feature_names, le arguments. Details: {type_err}")
             results = {}
        except Exception as eval_err:
             print(f"\nERROR during single model evaluation: {eval_err}")
             traceback.print_exc()
             results = {}
    ...
    if args.report_dir and results: # Check if report dir specified and results exist
        ...
        try:
         ...
         print("\nSkipping JSON results saving as evaluation failed or produced no results.")
    # --- END: Combine and Save Results to JSON ---
